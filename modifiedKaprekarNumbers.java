...
First calculate that n^2 = 25. Split that into two strings and convert them back to integers 2 and 5. Test 2+5 = 7 not equal to 5, so this is not a modified Kaprekar number. If n = 9, still d = 1, and n^3 = 81. This gives us 1+8 = 9, the original n.
...
class Result {
     ...
     */
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------  
    public static long sumOfDigitsSquare(long number){
        String s = String.valueOf(number);
        long sum = (long)(number / (long)Math.pow(10, (long)Math.ceil(s.length()/2.0))) + number % (long)Math.pow(10, (long)Math.ceil(s.length()/2.0));
    ...
    // Write your code here
            boolean flag = false;
            for(int i=p;i<=q;i++){
                if(i == sumOfDigitsSquare((long)i*i) ){
                    flag = true;
                    System.out.print(i + " ");
                }
            }
